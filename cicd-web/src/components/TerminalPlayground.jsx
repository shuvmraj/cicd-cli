...
// Helper Component to animate 3D properties inside R3F render loop
function SceneGroup({ animPhase, children }) {
  ...
  useFrame((state) => {
    ...
    // Linearly interpolate coordinates for 60fps GPU smoothness
...
export default function TerminalPlayground() {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          ...
          // Phase 1: Spin Y-axis 180 to 0 (top-down view)
          const revTimer = setTimeout(() => {
            setAnimPhase('revolving');
          }, 300);

          // Phase 2: Tilt camera view to flat straight-on horizontal
          const openTimer = setTimeout(() => {
            setAnimPhase('opening');
          }, 1500);

          // Phase 3: Display helper notification and begin console logs
          const readyTimer = setTimeout(() => {
            setAnimPhase('ready');
            setShowNotification(true);
          }, 2900);

          // Phase 4: Dismiss notification
  ...
  // Types terminal command output only after laptop is ready
  useEffect(() => {
